...
Svgout::Svgout(Configuration **configuration, wxString filename, int scale)
{
  ...
  m_tree = NULL;
  m_scale = scale;
  // The depth 24 hinders wxWidgets from creating rgb0 svgouts that some
...
bool Svgout::Layout(long int maxSize)
{
  ...
  *m_configuration = new Configuration(*m_dc);
  ...
  // Too big svgouts or svgouts that are too wide or high can crash windows
  // or the X server.
  if ((maxSize < 0) ||
      (
...
void Svgout::Draw()
{
  ...
  m_dc->SelectObject(wxNullSvgout);
  // Update the svgout's size information.
  m_ppi = m_dc->GetPPI();
  m_ppi.x *= m_scale;
  m_ppi.y *= m_scale;
...
wxSize Svgout::ToFile(wxString file)
{
  // Assign an resolution to the svgout.
  ...
  if (file.Right(4) == wxT(".bmp"))
    success = img.SaveFile(file, wxSVGOUT_TYPE_BMP);
  else if (file.Right(4) == wxT(".xpm"))
    success = img.SaveFile(file, wxSVGOUT_TYPE_XPM);
  else if (file.Right(4) == wxT(".jpg"))
    success = img.SaveFile(file, wxSVGOUT_TYPE_JPEG);
  else
  {
    if (file.Right(4) != wxT(".png"))
      file = file + wxT(".png");
    success = img.SaveFile(file, wxSVGOUT_TYPE_PNG);
  }
...
bool Svgout::ToClipboard()
{
  ...
  {
    bool res = wxTheClipboard->SetData(new wxSvgoutDataObject(m_bmp));
