...
public:
  ...
  */
  Svgout(Configuration **configuration, wxString filename, int scale = 1);

  ~Svgout();

  /*! Renders tree as svgout
  ...
  /*! Exports this svgout to a file

    \return The size of the svgout in millimeters. Sizes <0 indicate that the export has failed.
   */
  wxSize ToFile(wxString file);

  //! Returns the svgout representation of the list of cells that was passed to SetData()
  wxSvgout GetSvgout()
  { return m_bmp; }

  //! Copies the svgout representation of the list of cells that was passed to SetData()
  bool ToClipboard();
